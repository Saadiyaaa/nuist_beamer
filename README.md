...
├── README.md                   # 本文件
├── .gitignore
└── ASTRA2026_Presentation_Beaner_Template/    # 主模板
...
```bash
cd ASTRA2026_Presentation_Beaner_Template
latexmk main.tex
```
...
直接用 VS Code 打开整个模板目录 `ASTRA2026_Presentation_Beaner_Template/`，而不是只打开 `main.tex`。仓库中的 `.vscode/settings.json` 已将默认 recipe 设置为 `latexmk (XeLaTeX)`。
...
编译后打开 `main.tex`，替换其中的占位符内容即可。图片请使用相对路径，放在模板根目录或 `figures/` 目录，例如：
